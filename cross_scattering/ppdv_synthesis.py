...
class PPDVSyn:
    ...
    def synthesis(self, target_channel, ref_channel, image_init, optim_algorithm='LBFGS', 
        ...
        N_components = ref_channel.shape[0]
        ...
        for i in range(N_components):
            image_model.append(OptimizableImage(self.image_transform_inverse(image_init[[i]], target_channel)))

        # define optimizer
        params = [model.parameters() for model in image_model]
        if optim_algorithm   =='Adam':
            optimizer = torch.optim.Adam(itertools.chain(*params), lr=learning_rate)
        elif optim_algorithm =='NAdam':
            optimizer = torch.optim.NAdam(itertools.chain(*params), lr=learning_rate)
        elif optim_algorithm =='SGD':
            optimizer = torch.optim.SGD(itertools.chain(*params), lr=learning_rate)
        elif optim_algorithm =='Adamax':
            optimizer = torch.optim.Adamax(itertools.chain(*params), lr=learning_rate)
        elif optim_algorithm =='LBFGS':
            optimizer = torch.optim.LBFGS(itertools.chain(*params), lr=learning_rate, 
        ...
        def closure():
            ...
            image_syn = torch.cat([self.image_transform(image_model[i].image, target_channel) for i in range(N_components)], dim=0)
            image_residual = target_channel-image_syn.sum(0)
            image_syn = torch.cat([image_syn, image_residual], dim=0)

            # compute loss terms
            for c in range(N_components):
                self.st_calc.add_ref(ref_channel[[c]])
                self.st_calc.add_ref_ab(target_channel, ref_channel[[c]])
                estimator_model = self.estimator_func(image_syn[[c]])
                estimator_target = self.estimator_func(ref_channel[[c]])
                prior_loss += self.prior_loss(estimator_model, estimator_target)
                cross_loss += (self.cross_loss(self.estimator_func_cov(image_syn[[-1]], ref_channel[[c]])))
            prior_loss += self.prior_loss(self.estimator_func(image_syn[:-1].sum(0)[None,...]), self.estimator_func(ref_channel.sum(0)[None,...]))
            cross_loss += self.cross_loss(self.estimator_func_cov(image_syn[[-1]], ref_channel.sum(0)[None,...]))
